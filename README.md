...
- **An agent that is always on** — the agent is a daemon working a queue, not a batch job you start. A new watch is being hunted within seconds, and keeps being hunted on its own while it has open slots — backing off from 15 minutes to 6 hours while a site has nothing new, and stopping entirely once every slot is filled until one frees. On a full watch, "hunt for better" asks for one more look: a hunt that swaps out the weakest tracked listing only for something better, and in cheapest mode only for a strictly lower price. Tracked listings are re-read on their own every half hour; "hunt now" or "check prices" jump the queue; and a watch can switch its own hunting off. The Activity page shows every hunt's log live over SSE, what is queued next, and the history.
...
|---|---|
| [`backend/.env.example`](backend/.env.example) | `DATABASE_URL`, `JWT_SECRET` (generate one!), `COOKIE_SECURE`, `REGISTRATION_OPEN`, `OIDC_*`, `NTFY_SERVER_URL`, `VISION_SIDECAR_URL`, `MCP_ENABLED`, `RECHECK_INTERVAL_MINUTES` / `RECHECK_INTERVAL_FLOOR_MINUTES` and `HUNT_ENABLED` (same values as the agent's) |
| [`agent/.env.example`](agent/.env.example) | `AI_PROVIDER` / `AI_MODEL` / `AI_URL` / `AI_API_KEY`, `PLAYWRIGHT_MCP_URL`, `DATABASE_URL`, `SEAR_XNG_URL`, `EXPECTED_CURRENCY`, `VISION_SIDECAR_URL`, `NOTIFY_COOLDOWN_HOURS`, `RECHECK_INTERVAL_MINUTES` (the default a watch's own "Check every" overrides) / `RECHECK_INTERVAL_FLOOR_MINUTES` and the pool sizes, `HUNT_ENABLED` and the `HUNT_BACKOFF_*` wait, the `JOB_*` lifecycle caps, the `SITE_BREAKER_*` thresholds, the `PRICE_BAND_*` plausibility bands; optional LangSmith / Langfuse tracing |

**The panic button.** `HUNT_ENABLED=false` stops all hunting — nothing new is queued, queued hunts wait unclaimed, and "hunt now" answers 409 — while tracked prices keep being rechecked. Set it in **both** `backend/.env` and the agent's env (`agent/.env`, or `agent/.env.docker` under compose) and restart both: the agent is what stops, and the backend is what refuses "hunt now" and shows "hunting paused by the operator" in the UI.
