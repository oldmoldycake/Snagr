...
- **Ownership**: ALL data (categories, sites, items, listings, watches, runs) is private
  per-user. Every query must be scoped to the authenticated user; `user_id` never appears in
  request bodies or responses. Cross-user access is a 404, not a 403 (don't leak existence).
...
11. **New `agent_runs`** — `id PK, user_id FK NOT NULL, scope TEXT NOT NULL`
    (`global|category|site|item`), `scope_id INT NULL, status TEXT NOT NULL`
    ...
    Enforce **one active run per user** with a partial unique index:
    `CREATE UNIQUE INDEX one_active_run_per_user ON agent_runs (user_id) WHERE status IN ('queued','running');`
...
| 28 | POST | `/api/runs` | user | Trigger run → 202; 409 `run_in_progress` if one is active |
| 29 | GET | `/api/runs` | user | History, `?status&scope&page&per_page` |
| 30 | GET | `/api/runs/{id}` | user | Run detail with stats |
| 31 | GET | `/api/runs/{id}/events` | user | Event backfill, `?after_seq=0&limit=500` |
| 32 | POST | `/api/runs/{id}/cancel` | user | Cancel a queued/running run |
...
```json
{ "run": { "id": 88, "scope": "category", "scope_id": 3, "scope_label": "Category: GPUs",
...
```json
{ "id": 88, "scope": "category", "scope_id": 3, "scope_label": "Category: GPUs",
...
- `Content-Type: text/event-stream`, cookie-authenticated, one stream per user. Send
  `X-Accel-Buffering: no` and flush per event.
- **On every connect/reconnect**, immediately send the snapshot:

  ```
  event: run.snapshot
  data: {"active_runs": [{"id": 88, "status": "running", "scope": "category",
...
- Lifecycle events: `run.started`, `run.finished`, `run.failed`, each with `data: {"run": {…}}`
  ...
  runs (status `cancelled`); `run.failed` only for status `failed`.
- **Heartbeat**: comment line `: ping` every 15 seconds.
- **Reconnects**: the client does NOT rely on `Last-Event-ID` replay. It compares the snapshot's
  `last_seq` with the highest seq it has rendered and backfills the gap via
  `GET /api/runs/{id}/events?after_seq=N`. The server never needs to replay from the stream.

## 7. Runs & the agent

- `POST /api/runs` inserts a `queued` `agent_runs` row and hands it to the agent worker
  (background task / queue — your call). Return 202 immediately.
- One active run per user (the partial unique index above); concurrent trigger → 409 with the
  active `run_id`.
- The agent, as it works: sets `status='running'` + `started_at`; appends `run_events` rows with
  monotonically increasing `seq` (fanned out to the user's SSE stream); writes `price_checks`
...
- Scopes: `global` = all the user's active listings + discovery/evaluation for every item;
  `category` = that category's items across their effective sites; `site` = all listings on that
  site; `item` = that item's listings (+ discovery/evaluation).
