...
- **Comments earn their keep.** Explain *why* — domain rules, gotchas, decisions — never narrate what the code does. Preserve the `# + api` markers in `models.py`. Exception: docstrings in `agent/tools.py` are prompts (above).
