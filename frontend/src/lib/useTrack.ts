 ...
 * `target` by setting its edges (--l, --r) and which way it is moving
 * (data-dir). A change of target animates it (`animate`); the first placement
 * and a re-seat after the host changes width (fonts loading, a tab arriving)
 * jump instead.
 */
function seatTrack(host: HTMLElement, marker: HTMLElement, target: HTMLElement | null, animate: boolean) {
  ...
  if (!wasIdle && marker.style.getPropertyValue('--l') === `${left}px` && marker.style.getPropertyValue('--r') === `${right}px`) {
    return
  }
  ...
  if (!animate) {
    ...
    // grow out of the new target rather than slide in from wherever the marker was hidden
 ...
 * without animating when the host resizes. `from` names where a freshly
 * mounted marker starts, so it slides over from there instead of appearing.
 */
