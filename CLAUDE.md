...
**The queue is the design** (`agent/jobs.py`, `backend/app/services/jobs.py`, migration 015). A partial unique index allows at most one *open* job per target, so "check this listing now" is an UPDATE of the pending row rather than a second row, every insert is `ON CONFLICT DO NOTHING`, and `POST /api/jobs` never needs a 409 for a duplicate. A listing always has exactly one recheck ahead of it: completing one inserts the next in the same transaction, and so does failing one — a listing that dropped out of the queue would silently stop being watched. `claim` is one `UPDATE … WHERE id = (SELECT … FOR UPDATE SKIP LOCKED)` statement that is blind to paused sites. Rechecks recur at the watch's own `recheck_interval_minutes`, else `RECHECK_INTERVAL_MINUTES`, never below `RECHECK_INTERVAL_FLOOR_MINUTES` (set both in `backend/.env` too — the UI reads the default from it and the API refuses an interval below the floor).

**A watch with open slots is hunted on its own; a full one is not hunted at all** (decision 9). A finished hunt queues its pair's next one in the same transaction while the watch has room and `watches.hunt` is on: at once after a save, otherwise after `payload.backoff_minutes`, doubled each empty hunt from `HUNT_BACKOFF_MIN_MINUTES` to `HUNT_BACKOFF_CAP_MINUTES` (15 → 360, `reason='backoff'`). A freed slot — `disable_listing`, a recheck that sees the listing end, the user's untrack — wakes the watch's hunts with their backoff forgotten, and so does a user's "hunt now". The sweep (`jobs.sweep`, hourly and on the scheduler's first pass) is the safety net: a hunt for every pair with open slots, the switch on and nothing open. `watches.hunt = false` means hunted only on "hunt now"; rechecks carry on. `HUNT_ENABLED=false` is the operator's kill switch: no sweep, no successors, no wakes, and the hunt pool claims only `ground`, so queued hunts wait; the backend (same value in `backend/.env`) answers `POST /api/jobs {hunt}` with 409 `hunting_disabled`.

**A person's "hunt now" on a full watch is a swap hunt** (decision 10). The backend flags it `payload.swap = true` (`services/jobs.py::_hunts`; the sweep, successors and slot-freed wakes never do), and `run_hunt_job` swaps the `TRACKING SLOTS` prompt block for `TRACKED LISTINGS`: the watch's active listings weakest first (cheapest: highest price; best match: lowest score, price as tiebreak). The model picks the weakest with `disable_listing(id, reason="replaced")`, which only records the pick on `UnitContext.swap`, and then calls `save_listing` with the candidate's `price`. That save makes the trade: it retires the old listing and saves the new one in one transaction under a lock on the watch row, and records the price as the new listing's first check. Code is the backstop: `save_listing` on a full watch answers `SLOTS FULL:` without a pick, after the one swap per hunt, or in cheapest mode unless the price is strictly lower than the replaced listing's last confirmed one, and a refused save changes nothing. A `replaced` listing stays in `get_known_listing_urls` for `REPLACED_HIDDEN_FOR` (24 h, read from the swap's `listing_ended` event), and re-saving its URL later reactivates the same row with its history, unlike sold/ended/auction, which stay terminal.
