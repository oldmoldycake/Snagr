...
`main.py` → `agent.run()` (full sweep) and `agent.consume()` (queue tick) both land in `execute_run()`, which does a grounding pre-pass (`pricing.ground_stale()`, global-scope runs only, isolated — a failure is logged and the scrape continues ungrounded) and then two scrape passes: (1) re-check every active listing (`get_listed_items`) for price/availability; (2) scan sites for new listings for each notify-enabled `(watch, site)` pair (`get_watched_item_list`), skipping any pair whose watch has already filled its `max_listings` slots (`get_active_listing_count`), and skipping URLs successfully re-checked earlier in the same run plus previously-rejected ones (`get_checked_urls` → `listing_checks`). The tools in `agent/tools.py` are exposed to the LLM and split by pass — the recheck agent gets `save_price_check` + `disable_listing`, the scan agent `save_price_check` + `save_listing` + `log_listing_check` (plus `check_images` when `VISION_SIDECAR_URL` is set) — and **their docstrings are the tool descriptions the model reads**, so keep them accurate and imperative. **Notifications: the agent detects, the backend delivers.** `save_price_check` queues a `target.hit` and `save_listing` a `listing.new` into `notification_outbox` (migration 010's trigger wakes the backend dispatcher, `backend/app/services/notifications.py`, which fans out to the user's channels — ntfy/Discord/webhook — and owns retries). The agent writes events because it is the only writer of `price_checks`/`listings`; it never contacts a push service. `target.hit` is edge-triggered — only a price that *crosses* the watch's target enqueues, stamped on `watches.last_notified_at` at enqueue (`NOTIFY_COOLDOWN_HOURS` is the spam floor) — and never raises, because losing the observation to a failed enqueue would be worse than losing the notification. **A run is never left `running` by a dead process:** the driver heartbeats `agent_runs.heartbeat_at`, each tick first fails any run silent past `RUN_STALE_AFTER_SECONDS` (`reap_stale_runs`), SIGTERM/SIGINT fail the run in flight on the way out (`main.py` → `_supervised`, forwarded by `agent/ticker.sh` under compose), and every unit is capped by `AGENT_MAX_STEPS` and `AGENT_UNIT_TIMEOUT_SECONDS`. The provider is pluggable via `AI_PROVIDER`/`AI_MODEL`/`AI_URL`/`AI_API_KEY` (any LangChain `init_chat_model` provider).
