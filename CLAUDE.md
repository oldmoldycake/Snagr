...
`main.py` → `agent.run()` does two passes per invocation: (1) re-check every active listing (`get_listed_items`) for price/availability; (2) scan sites for new listings for each notify-enabled `(watch, site)` pair (`get_watched_item_list`), skipping already-known and previously-rejected URLs (`listing_checks`). The DB-writing tools in `agent/tools.py` are exposed to the LLM — **their docstrings are the tool descriptions the model reads**, so keep them accurate and imperative. `save_price_check` also fires the ntfy target-hit push (`agent/notify.py`, off unless `NTFY_SERVER_URL` is set): the agent owns it because it is the only writer of `price_checks`. It is edge-triggered — only a price that *crosses* the watch's target notifies, stamped on `watches.last_notified_at` — and never raises, because losing the observation to a wedged ntfy server would be worse than losing the push. The provider is pluggable via `AI_PROVIDER`/`AI_MODEL`/`AI_URL`/`AI_API_KEY` (any LangChain `init_chat_model` provider).
