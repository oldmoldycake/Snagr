...
async def cat_build(id: int, name: str, slug: str, db: AsyncSession) -> Category:
        ...
        stmt = select(SiteCategories.site_id
            ).where(SiteCategories.category_id == id)
        ...
        stmt = select(func.count(distinct(Listings.item_id))
...
async def create_category(body: CategoryCreateRequest, user=Depends(current_user),
    ...
    if await db.scalar(select(Categories).where(func.lower(Categories.name) == body.name.lower())):
        raise err(422, "validation_error", "Category already exsits",
                  fields={"name": "Category already exsits"})
...
async def set_category_sites(category_id: int, body: SetCategorySitesRequest,
    ...
    valid_ids = (await db.execute(
        select(Sites.id).where(Sites.id.in_(body.site_ids))
    )).scalars().all()
