...
Security fixes land on `main` and ship in the latest release only — there are no backports. Until the first tagged release, the supported artifacts are the `:dev` images built from the tip of `main`.
...
- Credentials: httpOnly cookie sessions (`snagr_access` JWT + a rotating DB-backed `snagr_refresh`), CSRF-header-guarded mutations, and `snagr_pat_…` API tokens (sha256 at rest, scoped read / write / runs, barred from `/api/auth/me`, `/api/me/*` and `/api/admin/*`). Bugs that cross any of those boundaries are in scope. `MCP_ENABLED=false` disables bearer-token authentication and unmounts `/api/mcp`; the token-management routes remain, but the tokens they mint authenticate nothing.
