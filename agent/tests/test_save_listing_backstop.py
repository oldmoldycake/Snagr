"""save_listing's deterministic vision backstop against a real Postgres: a
vision_scans row with auto_reject set refuses the save even if the model
ignored check_images' REJECT directive; without one, saves work as before.

Same harness rules as test_run_queue_db.py: conftest rewrites DATABASE_URL
to the throwaway snagr_test, one module-wide event loop (asyncpg
connections are loop-bound), schema built here as a test affordance (D1
still holds). Don't run concurrently with backend/tests.
"""

import asyncio

import pytest
import tools
from database import (
    Base,
    Categories,
    Items,
    Listings,
    Sites,
    User,
    VisionScans,
    Watches,
    engine,
)
from sqlalchemy import select, text

_ALL_TABLES = ", ".join(t.name for t in Base.metadata.sorted_tables)

_LOOP = asyncio.new_event_loop()


def db(coro):
    """Run one coroutine on the module's shared event loop."""
    return _LOOP.run_until_complete(coro)


async def _create_schema():
    async with engine.begin() as conn:
        for table in reversed(Base.metadata.sorted_tables):
            await conn.execute(text(f"DROP TABLE IF EXISTS {table.name} CASCADE"))
        await conn.run_sync(Base.metadata.create_all)


async def _drop_schema():
    async with engine.begin() as conn:
        for table in reversed(Base.metadata.sorted_tables):
            await conn.execute(text(f"DROP TABLE IF EXISTS {table.name} CASCADE"))


async def _truncate():
    async with engine.begin() as conn:
        await conn.execute(text(f"TRUNCATE {_ALL_TABLES} RESTART IDENTITY CASCADE"))


@pytest.fixture(scope="module", autouse=True)
def _schema():
    db(_create_schema())
    yield
    db(_drop_schema())
    db(engine.dispose())
    _LOOP.close()


@pytest.fixture(autouse=True)
def _clean_tables():
    yield
    db(_truncate())


async def _seed_watch(auto_reject: bool | None) -> dict:
    """user+item+site+watch, plus a vision scan when auto_reject isn't None."""
    from database import AsyncSessionLocal

    async with AsyncSessionLocal() as session:
        user = User(email="owner@test.local")
        category = Categories(name="Games", slug="games")
        site = Sites(name="TestBay", base_url="https://example.test")
        session.add_all([user, category, site])
        await session.flush()
        item = Items(category_id=category.id, name="Widget")
        session.add(item)
        await session.flush()
        watch = Watches(user_id=user.id, item_id=item.id)
        session.add(watch)
        await session.flush()
        if auto_reject is not None:
            session.add(
                VisionScans(
                    watch_id=watch.id,
                    listing_url="https://example.test/candidate",
                    auto_reject=auto_reject,
                )
            )
        ids = {"watch_id": watch.id, "item_id": item.id, "site_id": site.id}
        await session.commit()
        return ids


def _save(ids) -> int | str:
    return db(
        tools.save_listing(
            watch_id=ids["watch_id"],
            item_id=ids["item_id"],
            site_id=ids["site_id"],
            url="https://example.test/candidate",
            title="Widget — very real",
            match_score=80,
            match_summary="fits, no repro flags",
        )
    )


async def _listing_count() -> int:
    from database import AsyncSessionLocal

    async with AsyncSessionLocal() as session:
        return len((await session.execute(select(Listings))).scalars().all())


def test_auto_rejected_scan_refuses_the_save():
    ids = db(_seed_watch(auto_reject=True))
    tools.reset_run_stats()

    result = _save(ids)
    assert isinstance(result, str)
    assert result.startswith("REFUSED:")
    assert "log_listing_check" in result
    assert db(_listing_count()) == 0
    assert tools.read_run_stats()["new_listings"] == 0


def test_a_scan_below_threshold_saves_as_before():
    ids = db(_seed_watch(auto_reject=False))
    result = _save(ids)
    assert isinstance(result, int)
    assert db(_listing_count()) == 1


def test_no_scan_at_all_saves_as_before():
    ids = db(_seed_watch(auto_reject=None))
    result = _save(ids)
    assert isinstance(result, int)
